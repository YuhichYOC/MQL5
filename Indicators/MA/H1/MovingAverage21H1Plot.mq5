...
#ifndef D_MOVING_AVERAGE_21H1_PLOT_H
#define D_MOVING_AVERAGE_21H1_PLOT_H
...
#ifndef D_MOVING_AVERAGE_21H1_PLOT_B
#define D_MOVING_AVERAGE_21H1_PLOT_B
...
void MovingAverage21H1Plot::Initialize(int size) {
    ...
    m_period = _Period;
    m_size = size;
    m_m.Initialize(m_symbol, m_period, m_size, 20, 1);
}
...
void MovingAverage21H1Plot::Calc(int appendSize) {
    m_m.Calc(appendSize);
    m_size += appendSize;
}

void MovingAverage21H1Plot::Refresh(int size, double &plotMASeries[]) {
    if (size == 0) {
        m_m.Refresh();
